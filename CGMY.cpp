...
double MCFT1(double S0, double  K, double  T, double  r, double  C, double  G, double  M, double  Y, double q, double L, int N_sim);
...
{
	bool CallFlag = false;		//true for call option, false for put option
	bool splineFlag = false;	//spline interpolation in FFT and FST
	...
	int N_true_price = int(pow(2, 14)); //number of expansions for COS method
	double D = 10;				//range for CDF recovery in MCFT1 and MCFT2

	double alpha(1.5), dk(0.025); //Carr Madan parameters
	...
	if (errorFlag)
	{
		...
		{
			COS_CUDA[k] = MCFT1(S0, K_grid_10[k], T, r, C, G, M, Y, q, D / 2, N_sim);
			CDF_CUDA[k] = MCFT2(S0, K_grid_10[k], T, r, C, G, M, Y, q, D, N_sim);
		}
		...
		for (int k = 0; k < N_terms.size(); k++)
		{
			...
			for (int j = 0; j < K_grid_10.size(); j++)
			{
				...
				double COSprice = CGMYoption1.PriceByCOS(K_grid_10[j], T, N, 2 * D, CallFlag);
		...
		for (int k = 0; k < K_grid.size(); k++)
		{
			...
			COS_CUDA[k] = MCFT1(S0, K_grid[k], T, r, C, G, M, Y, q, D, N_sim);
			CDF_CUDA[k] = MCFT2(S0, K_grid[k], T, r, C, G, M, Y, q, D, N_sim);
		}
	...
	if (timeFlag)
	{
		...
		for (int t = 0; t < 10; t++)
		{
			...
			start = high_resolution_clock::now();
			double MCFT1price = MCFT1(S0, K, T, r, C, G, M, Y, q, D, N_sim);
			stop = high_resolution_clock::now();
			gpu1MonteCarlotime1 = min(gpu1MonteCarlotime1, duration_cast<milliseconds>(stop - start).count());

			start = high_resolution_clock::now();
			double MCFT2price = MCFT2(S0, K, T, r, C, G, M, Y, q, D, N_sim);
			stop = high_resolution_clock::now();
